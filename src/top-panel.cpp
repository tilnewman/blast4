...
namespace blast4
{
    ...
    void TopPanel::setup(Context & t_context)
    {
        M_CHECK(
            m_titleFont.openFromFile(
                (t_context.settings.media_path / "font/game-over/game-over.ttf").string()),
            "Failed to load title font");

        M_CHECK(
            m_generalFont.openFromFile(
                (t_context.settings.media_path / "font/d-din/d-din.ttf").string()),
            "Failed to load general font");

        m_windowSize = sf::Vector2f{ t_context.window.getSize() };
        m_boardRect = t_context.board.rect();

        const float boardBorderPad{ m_windowSize.x * t_context.settings.border_pad_ratio };
        ...
        m_titleText.setFillColor(t_context.settings.title_color);
        ...
        m_ammoRect.size.y = (m_windowSize.y * t_context.settings.text_size_ratio);
        ...
        m_ammoText.setFillColor(t_context.settings.text_color);
    ...
    void TopPanel::update(Context & t_context)
    {
        m_ammoText.setString(std::to_string(t_context.game.ammo));
        util::setOriginToPosition(m_ammoText);
        util::fit(m_ammoText, m_ammoRect);
        m_ammoText.setPosition(m_ammoRect.position);

        m_scoreText.setString(std::to_string(t_context.game.score));
    ...
    void TopPanel::draw(Context & t_context) const
    {
        t_context.window.draw(m_titleText);
        t_context.window.draw(m_ammoText);
        t_context.window.draw(m_scoreText);
    }
